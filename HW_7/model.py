...
def get_data() -> list:
    ...
    """
    with open("HW_7\gb.json", 'r', encoding="utf-8") as file:
...
def get_data_id(id: int) -> dict:
    ...
    """
    with open("gb.json", "r", encoding="utf-8") as file:
        data_file = json.load(file)
...
def get_data_last_name(last_name: str) -> list:
    """
    Возвращает одну запись по фамилии
    """
    res = []
    with open("gb.json", "r", encoding="utf-8") as file:
        data_file = json.load(file)
...
def add_data(data: dict):
    ...
    with open("HW_7\gb.json", "w", encoding="utf-8") as file:
